...
This platform provides a flexable `audio_adc` component for use with analogue microphones. It can be configured as either a `primary` or `secondary` for an `i2s_audio` source.
...
## Configuration variables
 ...
 - pin_reset (Optional, Pin Schema): The GPIO used to reset the chip. Defining this will have the side effect of increasing the setup priority of this instance so that if multiple chip's share the same GPIO this is before the others. There should be at least one TLV320ADC3101 device with this defined.
 ...
 - mclk (Optional, int): The frequency of the master clock. Default 4096000Hz.
 - address (Optional, int): The I²C adress. Defaults to 24
 - mic_gain (Optional, percent): The microphone gain setting for both microphones. This can be overidden by the setting in `channel_left` and/or `channel_left`. Default 100%
 - channel_left (Optional): Left channel configuration
   ...
   - micbias (Optional, enum): Bias voltage of the microphone. One of `none`, `2.0V`,`2.5V`, `AVDD`. Default `SEL4`
